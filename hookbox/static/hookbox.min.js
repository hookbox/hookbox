...
filePath:"jsio/base.js"},hookbox:{src:'jsio("from net import connect as jsioConnect");jsio("from net.protocols.rtjp import RTJPProtocol");exports.__jsio=jsio.__jsio;exports.logging=logging;logger.setLevel(0);exports.connect=function(d,a,b){d.match("/$")||(d+="/");a=new HookBoxProtocol(d,a,b);window.WebSocket?(jsioConnect(a,"websocket",{url:d.replace("http://","ws://")+"ws"}),a.connectionLost=bind(a,"_connectionLost","websocket")):(jsioConnect(a,"csp",{url:d+"csp"}),a.connectionLost=bind(a,"_connectionLost","csp"));return a};\nvar Subscription=Class(function(){this.init=function(d,a){this.channelName=a.channel_name;this.history=a.history;this.historySize=a.history_size;this.state=a.state;this.presence=a.presence;this.canceled=!1;this.publish=bind(d,"publish",this.channelName)};this.onPublish=function(){};this.onSubscribe=function(){};this.onUnsubscribe=function(){};this.onState=function(){};this.frame=function(d,a){switch(d){case "PUBLISH":if(this.historySize)for(this.history.push(["PUBLISH",{user:a.user,payload:a.payload}]);this.history.length>\nthis.historySize;)this.history.shift();this.onPublish(a);break;case "UNSUBSCRIBE":if(this.historySize)for(this.history.push(["UNSUBSCRIBE",{user:a.user}]);this.history.length>this.historySize;)this.history.shift();for(var b=0,c;c=this.presence[b];++b)if(c==a.user){this.presence.splice(b,1);break}this.onUnsubscribe(a);break;case "SUBSCRIBE":if(this.historySize)for(this.history.push(["SUBSCRIBE",{user:a.user}]);this.history.length>this.historySize;)this.history.shift();this.presence.push(a.user);this.onSubscribe(a);\nbreak;case "STATE_UPDATE":for(b=0;c=a.deletes[b];++b)delete this.state[c];for(c in a.updates)this.state[c]=a.updates[c];this.onState(a)}};this.cancel=function(){this.canceled||(logger.debug("calling this._onCancel()"),this._onCancel())};this._onCancel=function(){}});\nHookBoxProtocol=Class([RTJPProtocol],function(d){this.onOpen=function(){};this.onClose=function(){};this.onConnectionLost=function(){};this.onError=function(){};this.onSubscribed=function(){};this.onUnsubscribed=function(){};this.onMessaged=function(){};this.init=function(a,b,c){d(this,"init",[]);this.url=a;try{this.cookieString=c||document.cookie}catch(e){this.cookieString=""}this.connected=!1;this._subscriptions={};this._buffered_subs=[];this._publishes=[];this._messages=[];this._errors={};this._connectPayload=\nb;this.username=null};this.subscribe=function(a){this.connected?this.sendFrame("SUBSCRIBE",{channel_name:a}):this._buffered_subs.push(a)};this.publish=function(a,b){this.connected?this.sendFrame("PUBLISH",{channel_name:a,payload:JSON.stringify(b)}):this._publishes.push([a,b])};this.message=function(a,b){this.connected?this.sendFrame("MESSAGE",{name:a,payload:JSON.stringify(b)}):this._messages.push([a,b])};this.connectionMade=function(){logger.debug("connectionMade");this.transport.setEncoding("utf8");\n""===this.cookieString?void 0===this._connectPayload?this.sendFrame("CONNECT",{cookie_string:"",payload:this._connectPayload}):this.sendFrame("CONNECT",{cookie_string:"",payload:JSON.stringify(this._connectPayload)}):this.sendFrame("CONNECT",{cookie_string:this.cookieString,payload:JSON.stringify(this._connectPayload)})};this.frameReceived=function(a,b,c){logger.debug("received frame",b,c);switch(b){case "MESSAGE":this.onMessaged(c);break;case "CONNECTED":this.connected=!0;for(this.username=c.name;this._buffered_subs.length;)this.sendFrame("SUBSCRIBE",\n{channel_name:this._buffered_subs.shift()});for(;this._publishes.length;)this.publish.apply(this,this._publishes.splice(0,1)[0]);for(;this._messages.length;)this.message.apply(this,this._messages.splice(0,1)[0]);this.onOpen(c);break;case "SUBSCRIBE":c.user==this.username?(b=new Subscription(this,c),this._subscriptions[c.channel_name]=b,b._onCancel=bind(this,function(){this.sendFrame("UNSUBSCRIBE",{channel_name:c.channel_name})}),this.onSubscribed(c.channel_name,b),K=b):this._subscriptions[c.channel_name].frame(b,\nc);break;case "STATE_UPDATE":case "PUBLISH":a=this._subscriptions[c.channel_name];a.frame(b,c);break;case "UNSUBSCRIBE":a=this._subscriptions[c.channel_name];a.canceled=!0;a.frame(b,c);c.user==this.username&&(delete this._subscriptions[c.channel_name],this.onUnsubscribed(a,c));break;case "ERROR":this.onError(c);break;case "SET_COOKIE":document.cookie=c.cookie}};this._connectionLost=function(a,b,c){if(c)if(logger.debug("connectionLost"),this.connected=!1,"string"==typeof b.type&&("ServerUnreachable"==\nb.type||"ConnectionTimeout"==b.type))this.onConnectionLost(b.type);else this.onClose();else logger.debug("connectionFailed",a),"websocket"==a&&(logger.debug("retry with csp"),this.connectionLost=bind(this,"_connectionLost","csp"),jsioConnect(this,"csp",{url:this.url+"csp"}))};this.disconnect=function(){this.transport.loseConnection()}});\n',
